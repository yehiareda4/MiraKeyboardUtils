...
public class MiraJKeyboardUtils implements ViewTreeObserver.OnGlobalLayoutListener {

    private final static int MAGIC_NUMBER = 200;
    private static final HashMap<SoftKeyboardToggleListener, MiraJKeyboardUtils> sListenerMap = new HashMap<>();
    private SoftKeyboardToggleListener mCallback;
    private final View mRootView;
    private Boolean prevValue = null;
    private final float mScreenDensity;

    private MiraJKeyboardUtils(Activity act, SoftKeyboardToggleListener listener) {
        mCallback = listener;
    ...
    public static void addKeyboardToggleListener(Activity act, SoftKeyboardToggleListener listener) {
        ...
        sListenerMap.put(listener, new MiraJKeyboardUtils(act, listener));
    }
    ...
    public static void removeKeyboardToggleListener(SoftKeyboardToggleListener listener) {
        if (sListenerMap.containsKey(listener)) {
            MiraJKeyboardUtils k = sListenerMap.get(listener);
            k.removeListener();
